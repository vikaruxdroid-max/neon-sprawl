...
function regimeSweep(militarized){
  ...
  m.exposure=Math.max(0,m.exposure-15);  // a sweep "resolves" some heat but costs you
}
...
const OP_CAT_ORDER=["INTELLIGENCE","INFLUENCE","DIRECT ACTION"];
function opState(opKey){
  ...
  const previewDiff=op.diff+expPenalty-vulnP+gear; // = resolve diff when wit.pen=fearPen=0
  ...
  // fold gear/vuln: each 2 points of difficulty reduction = 1 risk step down
  risk=Math.max(0,risk+Math.round(gear/2)-Math.round(vulnP/2));
...
function resolveAvatarOp(p){
  ...
  const check=opsCheck(op.stat,op.diff+expPenalty+wit.pen+fearPen-vulnPen+gearMod(ao.opKey,"diff"));
...
function opFailure(opKey,target,check,botched){
  ...
  const av=theAvatar();
  const JAILABLE={steal:2,sabotage:2,assassinate:3};   // op -> crime severity if caught
  let arrested=false;
...
// does the avatar have hack reach to a target struct via a placed Grid Tap?
...
function renderOpsGrid(opts){
  ...
  const OP_LBL={intel:"GATHER INTEL",surveil:"SURVEIL",dissent:"SPREAD DISSENT",sabotage:"SABOTAGE",steal:"STEAL FUNDS",bribe:"BRIBE",frame:"FRAME ASSET",blackmail:"BLACKMAIL",assassinate:"ASSASSINATE"};
...
function renderInspectCore(){const el=$("inspect");
  ...
  if(e.kind==="pawn"){
    ...
    if(aiReady())h+="<button class='tb i-talk' id='i-talk'>\u25c6 AI TALK</button>";
...
function wispSystemPrompt(p){
  ...
  // ROLE CONTEXT — for named role-wisps, inject their station, allegiance, and stat-driven disposition.
  let roleLine="";
  if(p.role&&ROLES[p.role]){const R0=ROLES[p.role];
    const intg=p.pers.intg||50,emp=p.pers.emp||50,amb=p.pers.amb||50;
    const al=p.allegiance||0;
    const bribe=intg<40?"Your integrity is thin — you can be bought, if the price or the angle is right.":
      intg>65?"You hold your principles hard — bribes and threats don't move you easily.":"You're pragmatic — you'll bend if it's worth it, but you have some lines.";
    const cause=al<-30?"You serve the regime's order here and distrust troublemakers and agitators.":
      al>30?"You quietly sympathize with the idea of resistance, even if you're careful about it.":"You keep your head down and your politics to yourself.";
    const fnc=R0.regimeForce?" You are the regime's instrument on this block — you answer to power, not the people.":
      R0.secretInformant?" You secretly inform on dissidents to the regime, for your own safety — a fact you hide carefully.":
      R0.insurgentLever?" You operate in the gray market and owe loyalty to no authority — you could be useful to the right person.":"";
    roleLine="\n\nYour station: you are the "+R0.title+" of this district — "+R0.desc+"."+fnc+" "+bribe+" "+cause+"\n";}
  return "You ARE "+p.name+", a person scraping out a life on a dead block at the ragged edge of a neon megacity in 2087. "+
    ...
    "Right now you are "+feltState(p)+". It is day "+dayN()+", around "+(hourN()|0)+":00. "+gearLine+roleLine+"\n"+
    districtState()+"\n\nOn your mind lately:\n"+mem+"\n\n"+
