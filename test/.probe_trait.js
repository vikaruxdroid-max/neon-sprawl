...
function mockPawn(intgVal,discovered){
  const p={kind:'pawn',pers:{intg:intgVal},intelSeen:{},isAvatar:false,hp:50};
  if(discovered)p.intelSeen.intg=true;
  return p;
}

// 1-5 as before
check('1 undiscovered intg=90 → 0', traitMod(mockPawn(90,false),'bribe')===0);
check('2 discovered intg=90 → +2', traitMod(mockPawn(90,true),'bribe')===2);
check('3 discovered intg=10 → -2', traitMod(mockPawn(10,true),'bribe')===-2);
check('4 discovered intg=50 → 0',  traitMod(mockPawn(50,true),'bribe')===0);
const pd=mockPawn(10,true);
check('5a surveil → 0',     traitMod(pd,'surveil')===0);
check('5b steal → 0',       traitMod(pd,'steal')===0);
check('5c blackmail → 0',   traitMod(pd,'blackmail')===0);
check('5d assassinate → 0', traitMod(pd,'assassinate')===0);

// 6. preview/resolve consistency
newGame(); ST.nextEv=1e9;
M().intel=50; M().exposure=0;  // give avatar enough intel, zero exposure
const realPawn=ST.pawns.find(p=>!p.isAvatar&&p.hp>0);
if(realPawn){
  ...
  const expPen=Math.floor((M().exposure||0)/25);   // = 0
  const baseDiff=AVATAR_OPS.bribe.diff;
  // vuln=0 (comfortable pawn), gear=0 (no gear), trait=+2 (intg=90 discovered)
  const expected=baseDiff+expPen+2;
  check('6 previewDiff formula', prev.diff===expected,
    'ok='+prev.ok+' got='+prev.diff+' want='+expected+' reason='+(prev.reason||''));
  const tgt=realPawn; tgt.kind='pawn';
  check('6 resolve traitMod matches preview', traitMod(tgt,'bribe')===2);
}else{
  fails++;console.log('6 SKIP no pawn');
}

// 7. bribeReceptivity regression
const pHigh={kind:'pawn',pers:{intg:90},intelSeen:{intg:true},isAvatar:false,hp:50};
const pLow ={kind:'pawn',pers:{intg:10},intelSeen:{intg:true},isAvatar:false,hp:50};
const rH=bribeReceptivity(pHigh), rL=bribeReceptivity(pLow);
check('7a receptivity 0..1', rH>=0&&rH<=1, 'got='+rH.toFixed(3));
check('7b high-intg less receptive', rH<rL, 'high='+rH.toFixed(3)+' low='+rL.toFixed(3));

if(fails>0) console.error(fails+' PROBE(S) FAILED'); else console.log('ALL PROBES PASS');
